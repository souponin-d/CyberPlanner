...
# --- Определение констант для среды ---
...
class PolyclinicEnv(gym.Env):
    ...
    def _process_patient_arrivals(self):
        ...
        """
        # Поток клиентов 2 в среднем за 5 минут, значит, arrival_rate = 2.0
    ...
    def step(self, action):
        ...
        # Штраф за время ожидания (для каждого ожидающего пациента)
        # Увеличено до 5.0
        waiting_penalty = 0
        for p in self.waiting_patients:
            waiting_penalty += 5.0 # Значительный штраф за каждый шаг ожидания
            p.waiting_time += 1 
...
def evaluate_agent(model, env_fn, num_episodes=100, is_sb3_model=True):
    """
...
if __name__ == '__main__':
    # --- 1. Пропускаем оптимизацию гиперпараметров Optuna и используем ранее найденные ---
    print("--- Пропуск оптимизации гиперпараметров Optuna. Используем предыдущие лучшие параметры ---")

    # Лучшие гиперпараметры, полученные из предыдущей оптимизации Optuna
    best_params = {
        'learning_rate': 5.3363745406452854e-05,
        'n_steps': 799.1206161053364, # Будет преобразовано в int
        'batch_size': 512,
        'gamma': 0.9699360758977167,
        'gae_lambda': 0.9250457980828103,
        'ent_coef': 1.0602957310040156e-07,
        'vf_coef': 0.5792320816082553
    }
    
    # Директория для логов TensorBoard и сохранения модели
    log_dir = "ppo_training_1M_steps" 
    os.makedirs(log_dir, exist_ok=True)

    # --- 2. Обучение PPO модели на 1 миллион шагов с заданными параметрами ---
    print("\n--- Обучение PPO модели на 1,000,000 шагов ---")
    
    # Параметры среды для обучения и оценки
    # arrival_rate = 2.0 (это 2 клиента в среднем за 5 минут, как запрошено)
    env_args = {'arrival_rate': 2.0, 'emergency_ratio': 0.2} 
    train_env = make_vec_env(PolyclinicEnv, n_envs=4, env_kwargs=env_args, vec_env_cls=SubprocVecEnv) 

    # Рассчитываем log_interval так, чтобы лог выводился только один раз в конце обучения
    log_interval_value = math.ceil(1_000_000 / (int(best_params['n_steps']) * 4)) # 4 - количество envs
    log_interval_value = max(1, log_interval_value) # Убедимся, что log_interval_value минимум 1
    ...
    best_ppo_model = PPO(
        ...
        verbose=1, # Включаем verbose для отображения прогресс-бара и финального лога
        tensorboard_log=log_dir, # <--- ТЕПЕРЬ ЭТОТ ПАРАМЕТР ЕСТЬ ЗДЕСЬ
        device='auto'
    )

    print("Начало финального обучения PPO на 1,000,000 шагов...")
    best_ppo_model.learn(total_timesteps=1_000_000, log_interval=log_interval_value)
    model_path = os.path.join(log_dir, "ppo_polyclinic_model_1M_steps")
    best_ppo_model.save(model_path)
    print(f"PPO модель сохранена по пути: {model_path}")
    train_env.close()

    # --- 3. Инициализация и оценка FIFO агента ---
    print("\n--- Оценка FIFO агента ---")
    fifo_env_creator = lambda: PolyclinicEnv(arrival_rate=2.0, emergency_ratio=0.2) # Обновленные параметры среды
    fifo_agent = FIFOAgent(fifo_env_creator()) 

    fifo_metrics = evaluate_agent(fifo_agent, fifo_env_creator, num_episodes=100, is_sb3_model=False) 
    print("Метрики FIFO агента:")
    for key, value in fifo_metrics.items():
        print(f"  {key}: {value:.4f}")

    # --- 4. Оценка обученной PPO модели ---
    print("\n--- Оценка обученной PPO модели ---")
    ppo_eval_env_creator = lambda: PolyclinicEnv(arrival_rate=2.0, emergency_ratio=0.2) # Обновленные параметры среды
    ppo_metrics = evaluate_agent(best_ppo_model, ppo_eval_env_creator, num_episodes=100, is_sb3_model=True) 
    print("Метрики PPO агента:")
    for key, value in ppo_metrics.items():
        print(f"  {key}: {value:.4f}")

    # --- 5. Сравнение результатов ---
    print("\n--- Сравнение PPO vs FIFO ---")
